...
namespace CSparse.Complex.Preconditioner
{
    ...
    /// </remarks>
    public sealed class MILU0 : IPreconditioner<Complex>
    {
        ...
        public MILU0(Matrix<Complex> matrix, bool symmetric = false)
        {
            var storage = matrix as CompressedColumnStorage<Complex>;

            if (storage == null)
            {
                throw new ArgumentException("Expected matrix of type CompressedColumnStorage<Complex>.");
            }
            ...
            var T = symmetric ? storage :  storage.Transpose(true);
        ...
        private int Compute(int n, Complex[] ax, int[] ai, int[] ap, Complex[] lux, int[] lup, int[] diag, bool modified)
        {
            var iw = new int[n];
            int i;
